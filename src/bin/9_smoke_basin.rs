                ...
                .map(|(row, cols)|
                    cols.iter()
                        ...
                        .map(|(col, height)| {
                            ...
                            if is_low_point { "🭭 "} else {"██"}.color(Color::TrueColor {
                                r: 0,
                                g: if is_low_point {
                                    255
                                } else {
...
fn trace_low_point(map: &[Vec<usize>], current_position: (usize, usize)) -> HashSet<(usize, usize)> {
    ...
    if current_height == 9 {
        ...
        if current_position.0 + 1 < map.len() && map[current_position.0 + 1][current_position.1] > current_height {
            result.extend(trace_low_point(map, (current_position.0 + 1, current_position.1)));
        }
        if current_position.1 + 1 < map[current_position.0].len() && map[current_position.0][current_position.1 + 1] > current_height {
            result.extend(trace_low_point(map, (current_position.0, current_position.1 + 1)));
        }
        if current_position.0 != 0 && map[current_position.0 - 1][current_position.1] > current_height {
            result.extend(trace_low_point(map, (current_position.0 - 1, current_position.1)))
        }
        if current_position.1 != 0 && map[current_position.0][current_position.1 - 1] > current_height {
            result.extend(trace_low_point(map, (current_position.0, current_position.1 - 1)))
        }
        result
    }
...
fn find_basins(input: &[Vec<usize>], low_points: Vec<(usize, usize)>) -> BTreeMap<(usize, usize), usize> {
...
fn main() {
    ...
    println!("Risk: {}", result);
    let basins = find_basins(&input, low_points.iter().map(|point| (point.row, point.col)).collect());
    let basin_frequencies = basin_frequencies(basins);
    let mut basin_sizes: Vec<usize> = basin_frequencies.values().copied().collect();
    basin_sizes.sort_unstable();
    println!("Result: {}", basin_sizes.iter().rev().take(3).product::<usize>());
}
// wrong: 1103780

#[cfg(test)]
mod tests {
    use crate::{read_input_to_map, find_low_points, find_basins, basin_frequencies};
